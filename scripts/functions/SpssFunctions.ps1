...
function AddSpssFilesToInstrumentPackage {
    ...
    # Generate SPS file
    try {
        & cmd.exe /c $tempPath\Manipula.exe "$tempPath\GenerateStatisticalScript.msux" -K:meta="$instrumentName.bmix" -H:"" -L:"" -N:oScript="$instrumentName,iFNames=,iData=$instrumentName.bdix" -P:"SPSS;;;;;;$instrumentName.asc;;;2;;64;;Y" -Q:True
    ...
    # Generate .ASC file
    try {
        & cmd.exe /c $tempPath\Manipula.exe "$tempPath\ExportData_$instrumentName.msux" -A:True -Q:True
